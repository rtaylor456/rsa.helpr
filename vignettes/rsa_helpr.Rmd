...
#### `handle_date` (vector input)
```{r handle_date}

```

#### `handle_excel_date` (vector input)
```{r handle_excel_date}
...
```{r unidentified_to_0}
rsa_clean <- clean_utah(rsa, unidentified_to_0 = TRUE)
head(rsa_clean$E42_Has_Disability_911)

rsa_clean <- clean_utah(rsa, unidentified_to_0 = FALSE)
head(rsa_clean$E42_Has_Disability_911)
...
rsa_clean <- clean_utah(rsa, remove_strictly_na = FALSE)

na_cols <- names(rsa_clean)[sapply(rsa_clean, function(x) all(is.na(x)))]
na_cols

rsa_clean <- clean_utah(rsa, remove_strictly_na = TRUE)
na_cols <- names(rsa_clean)[sapply(rsa_clean, function(x) all(is.na(x)))]
na_cols
```
...
scores_clean <- clean_scores(scores, state_filter = "Utah", clean_ID = TRUE)
...
scores_clean <- clean_scores(scores, aggregate = TRUE)
dim(scores_clean)
...
scores_clean <- clean_scores(scores, ID_col = "X")
